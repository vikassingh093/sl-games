    ...
    class User extends \Illuminate\Foundation\Auth\User implements \Tymon\JWTAuth\Contracts\JWTSubject
    {
        ...
        public function addBalance($type, $summ, $description = '', $payeer = false, $refund = true, $system = 'handpay', $update_level = true, $model = false)
        {
            ...
            if( $payeer->hasRole('cashier') && $this->hasRole('user') && $type == 'add' && $happyhour && $system == 'handpay' ) 
            {
                ...
                // $happyMul = intval(str_replace('x', '', $happyhour->multiplier));
                // Statistic::create([
                //     'user_id' => $this->id, 
                ...
                //     'hh_multiplier' => $happyMul, 
                //     'sum2' => $summ, 
                //     'shop_id' => ($this->hasRole('user') ? $this->shop_id : 0)
                // ]);
                // $this->increment('balance', $balance);
                // $this->increment('happyhours', $summ * $happyMul);
                // $this->increment('count_happyhours', $summ * $happyMul * $happyhour->wager);
                // $this->increment('address', $summ * $happyMul - $summ);
                // if( $type == 'out' ) 
                // {
            ...
            else
            {
                ...
                if($this->hasRole('user'))
                {
                    if($type == 'add')
                    {
                        ...
                        // $cashback = $balance * 0.2;
                        // $this->increment('balance_cashback', $cashback);
                        $this->increment('reward_base', $balance);
                        //check sunday funday
                        $today = date("Y-m-d");
                        ...
                        if($isSunday)
                        {
                            ...
                            if($sunday_bonus > 0)
                            {
                                \VanguardLTE\Statistic::create([
                                    ...
                                    'sum' => $sunday_bonus, 
                                    'last_balance' => $this->balance,
                                    'result_balance' => $this->balance + $sunday_bonus,
                                    'last_payeer_balance' => 0,
                                    'result_payeer_balance' => 0,
                                    'hh_multiplier' => 0, 
                                    'sum2' => $sunday_bonus, 
                                ...
                                $this->increment('sunday_funday_limit', $sunday_bonus * 3);
                                $this->increment('balance', $sunday_bonus);
                            }
                            */
                        }
                    }
