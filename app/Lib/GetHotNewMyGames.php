...
class GetHotNewMyGames
{
    ...
    public static function get_new_games($finded = false){
        ...
        if (Cache::has('new_games:'. $shop_id .':'.$is_mobile)) {
            ...
            $random_20_games = [];
            $last_30_games = Game::where(['view' => 1, 'shop_id' => 0]);
            // dd($last_30_games);
            if( $is_mobile ){
            ...
            $last_30_games = $last_30_games->orderBy('id', 'DESC')
                ...
                ->get();
            if( $last_30_games ){
                $game_count = $last_30_games->count() > 20 ? 20 : $last_30_games->count();
                $random_20_games = $last_30_games->random($game_count)->pluck('id');
    ...
    public static function get_my_games($finded = false){
        ...
        $my_games_stat = StatGame::where('user_id', (\Illuminate\Support\Facades\Auth::check() ? auth()->user()->id : 0))->groupBy('game')->take(20)->pluck('game');
        if($my_games_stat){
            $my_games = Game::where(['view' => 1, 'shop_id' => (\Illuminate\Support\Facades\Auth::check() ? auth()->user()->shop_id : 0)])->whereIn('name', $my_games_stat);
            $detect = new MobileDetect;
            if( $detect->isMobile() || $detect->isTablet() ){
