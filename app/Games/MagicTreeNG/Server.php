                    ...
                    try
                    {
                        ...
                        switch( $reqId ) 
                        {
                            ...
                            case 'CollectionBonusItemRequest':
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if($minWin == -1 || ($minWin > $allwin && $allwin > 0))
                                        {
                                            $minWin = $allwin;
                                            $minJArr = $jArr;
                                            $minPrize = $Prizes;
                                            $minCurrentSelect = $jArr[0];
                                        }
                                ...
                                if($isPicked == false)
                                {
                                    //win amount is still 0 even it's last pick
                                    $winAmount = $minWin;
                                    $jArr = $minJArr;
                                    $Prizes = $minPrize;
                                    $currentSelect = $minCurrentSelect;
                                }
                                $pickCount--;
                                ...
                                if( $pickCount == 0 ) 
                                {
                                    ...
                                    $slotSettings->SetWin($winAmount + $reserve);
                                    $slotSettings->SetBalance($winAmount);
                                    $slotSettings->SaveLogReport('NULL', $allbet, $winAmount, 'BG');
                                    $BonusState = 0;
                                    $state = 'Ready';
                                    $lastPick = 'true';
                                    $picksRemain = 0;
                            ...
                            case 'SpinRequest':
                                 ...
                                 * wild foo dog: 0
                                 * wild coin: 1
                                 * fortune cat scatter: 11
                                 * wild fortune cat: 12
                                 * yinyang scatter: 13
                                 * wild yinyang scatter: 14
                                 */
                                ...
                                for( $i = 0; $i <= $try; $i++ ) 
                                {
                                    ...
                                    {
                                        $wildCoinCnt = rand(3,7);
                                        $placedCnt = 0;
                                        while($placedCnt < $wildCoinCnt)
                                        {
                                            $r = rand(1, 5);
                                            $c = rand(0, 2);
                                            if($reels['reel'.$r][$c] != $scatter)
                                            {
                                                if($postData['slotEvent'] == 'freespin')
                                                {
                                                    if($reels['reel'.$r][$c] == '11') //convert to wild cat scatter                                                    
                                                        $reels['reel'.$r][$c] == '12';
                                                    
                                                    else if($reels['reel'.$r][$c] == '13') //convert to wild yinyang scatter
                                                        $reels['reel'.$r][$c] == '14';
                                                    else
                                                        $reels['reel' . $r][$c] = 1;    
                                                }
                                    ...
                                    for( $r = 1; $r <= 5; $r++ ) 
                                    {
                                        ...
                                        for( $p = 0; $p <= 2; $p++ ) 
                                        {
                                            if( $reels['reel' . $r][$p] == $scatter || $reels['reel' . $r][$p] == $wildScatter) 
                                            {
                                                $scattersCount++;
                                                $scattersPos[] = '["' . ($r - 1) . '","' . $p . '"]';
                                                if($reels['reel' . $r][$p] == $wildScatter)
                                                    $wildScatterCount++;
                                                $isScat = true;
                                            }
                                            if( $reels['reel' . $r][$p] == $scatterCat || $reels['reel' . $r][$p] == $wildScatterCat ) 
                                            {
                                                $scattersCatCount++;
                                                if($reels['reel' . $r][$p] == $wildScatterCat)
                                                    $wildScatterCatCount++;
                                                $scattersPos2[] = '["' . ($r - 1) . '","' . $p . '"]';
                                                $isScat = true;
                                            }
                                    ...
                                    if($scattersCount >= 3 && $winType != 'bonus')
                                        continue;
                                    $scattersWin = $slotSettings->Paytable['SYM_' . $scatter][$wildScatterCount] * $betLine * $lines * $bonusMpl;
                                    $scatters2Win = $slotSettings->Paytable['SYM_' . $scatterCat][$wildScatterCatCount] * $betLine * $lines * $bonusMpl;
                                    $gameState = 'Ready';
                                    ...
                                    if( $scattersCatCount >= 3 && $slotSettings->slotBonus ) 
                                    {
                                        $gameState = 'PickBonus';
                                    ...
                                    if($postData['slotEvent'] == 'freespin')
                                    {
                                        if($minTotalWin == -1 || $totalWin < $minTotalWin)
                                        {
                                            ...
                                            $minScatterCnt = $scattersCount;
                                            $minScatter2Cnt = $scattersCatCount;
                                            $minReels = $reels;
                                            $minTmpReels = $reelsTmp;
                                    ...
                                    else if($winType == 'none' && $totalWin == 0 && $scattersCatCount == 0)
                                    {
                                        break;
                                    }
                                }
                                ...
                                if($totalWin > 0 && $spinAcquired == false)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        $reels = $minReels;
                                        $reelsTmp = $minTmpReels;
                                        $scattersCount = $minScatterCnt;
                                        $scattersCatCount = $minScatter2Cnt;
