            ...
            try
            {
                ...
                switch( $reqId ) 
                {
                    ...
                    case 'SpinRequest':
                        ...
                        for( $i = 0; $i <= 500; $i++ ) 
                        {
                            ...
                            for( $k = 0; $k < 5; $k++ ) 
                            {
                                ...
                                for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                {
                                    ...
                                    if( $csym == $scatter || $csym == $sevenSym || !isset($slotSettings->Paytable['SYM_' . $csym]) ) 
                                    {
                                    }
                                    else
                                    {
                            ...
                            for( $r = 1; $r <= 5; $r++ ) 
                            {
                                for( $p = 0; $p <= 2; $p++ ) 
                                {
                                    if( $reels['reel' . $r][$p] == $sevenSym ) 
                                    {
                                        ...
                                        $tmpWin = $slotSettings->Paytable['SYM_' . $sevenSym][$cnt] * $betLine;
                                        if( $cWins[6] < $tmpWin ) 
                                        {
                                            $cWins[6] = $tmpWin;
                                            $tmpStringWin = '{"type":"LineWinAmount","selectedLine":"6","amount":"' . $tmpWin . '","wonSymbols":'.json_encode($wonSymbols).'}';
                                        }
                                    }
                                }
                            }

                            if($cWins[6] > 0 && $tmpStringWin != '')
                            {
                                array_push($lineWins, $tmpStringWin);
                                $totalWin += $cWins[6];
                            }
                        ...
                        if( count($lineWins) > 0 ) 
                        {
                            $winString = ',"slotWin":{"totalWin":"' . $totalWin . '","lineWinAmounts":[' . implode(',', $lineWins) . '],"canGamble":"true", "gambleParams":{"history":[1,1,1,1,1]}}';
                        }
                        else
                        {
                            $winString = '';
