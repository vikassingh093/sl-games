                ...
                \DB::transaction(function() use ($request, $game)
                {
                    ...
                    switch( $aid ) 
                    {
                        ...
                        case 'subscribe':
                            ...
                            shuffle($hist);
                            $slotSettings->SetGameData('FireFly1x2Cards', $hist);
                            $lastEvent = $slotSettings->GetHistory();
                            $slotSettings->SetGameData('FireFly1x2BonusWin', 0);
                            $slotSettings->SetGameData('FireFly1x2FreeGames', 0);
                            $slotSettings->SetGameData('FireFly1x2CurrentFreeGame', 0);
                            $slotSettings->SetGameData('FireFly1x2TotalWin', 0);
                            $slotSettings->SetGameData('FireFly1x2FreeBalance', 0);
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'bet':
                            ...
                            for( $i = 0; $i <= 2000; $i++ ) 
                            {
                                ...
                                else
                                {
                                    ...
                                    for($a = 0; $a < count($drawnNumbers); $a++)
                                    {
                                        $drawnNumbersStr[] = '<b num=\'' . $drawnNumbers[$a] . '\'/>';
                                    }
                                }
                                ...
                                if($minTotalWin == -1 || $minTotalWin > $totalWin)
                                {
                                    ...
                                    $minDrawnStr = $drawnNumbersStr;
                                }

                                $spinAcquired = false;

                                if( $totalWin <= $bank && $totalWin > 0.5 * $bank && $winType != 'none')
                                {
                                    $spinAcquired = true;
                                    break;
                                }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('FireFly1x2TotalWin', $totalWin);
                            $slotSettings->SetGameData('FireFly1x2GambleStep', 5);
                            $hist = $slotSettings->GetGameData('FireFly1x2Cards');
                            $result_tmp[0] = '<xml><betPlaced  errorCode=\'0\'  error_description=\'\' errorIn=\'PLACE\'/><ballsDrawn>' . implode('', $drawnNumbersStr) . '</ballsDrawn><ballsMatched num=\'' . count($matchNumbers) . '\' balls=\'' . implode(',', $matchNumbers) . '\'/><return   amount=\'' . ($totalWin * 100) . '\'/><newBalance  amount=\'' . ($totalWin * 100) . '\' /><message show=\'false\' ></message></xml>';
                            break;
                    }
