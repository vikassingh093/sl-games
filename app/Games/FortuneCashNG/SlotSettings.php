        ...
        public function GetReelStrips($winType, $slotEvent)
        {
            ...
            else
            {
                ...
                foreach($arrReels as $index => $reelStrip ) 
                {
                    if( is_array($this->$reelStrip) && count($this->$reelStrip) > 0 ) 
                    {
                        $prs[$index + 1] = $this->GetRandomScatterPos($this->$reelStrip, $sb);
                        $reelsId[] = $index + 1;
                    }
                }
                $reelsId = [
                ...
                for( $i = 0; $i < count($reelsId); $i++ ) 
                {
                    if( $i == 0 || $i == 2 || $i == 4 ) 
                    {
                        $prs[$reelsId[$i]] = $this->GetRandomScatterPos($this->{'reelStrip' . $reelsId[$i]}, $sb);
                    }
                    else
                    {
        ...
        public function GetNoWinSpin($slotEvent)
        {
            ...
            while($isWin)
            {
                ...
                if($win == 0)
                {
                    ...
                    if($scattersCount < 2 && $moneyBagCount < 2)
                        $isWin = false;
                }
