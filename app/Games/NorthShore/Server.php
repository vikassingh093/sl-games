            ...
            try
            {
                ...
                switch($type)
                {
                    ...
                    case "subLogin":
                        ...
                        $slotSettings->SetGameData('gameType','normal'); //reset game status normal,freespin,bonus
                    ...
                    case "spin":                        
                        ...
                        $bounsId = 0;  //0 :normal game 1: freespin 2: Fire Link
                        $freeSpinTotalTimes = 0;
                        $freeSpinTimes = 0;
                        ...
                        for($i = 0; $i <= 2000 ; $i++)
                        {
                            ...
                            switch($gameType)
                            {
                                case "normal":
                                    ...
                                    {
                                        $reels = $slotSettings->GetReelStrips('freespin',0); //generate freespin reel
                                        $slotSettings->SetGameData('totalGuess',$winTypeTmp[1]);
                                    }else if ($winTypeTmp[0] == "bonus")
                                    {
                                        $reels = $slotSettings->GetReelStrips('bonus',0); //generate bonus reel
                                        $slotSettings->SetGameData('totalGuess',$winTypeTmp[1]);
                                    }
                                    break;
                                case "freespin":
                                    $reels = $slotSettings->GetReelStrips('none',1); //reel generation during freespin
                                    break;
                                case "bonus":
                                    $reels = $slotSettings->GetReelStrips('none',2); //reel generation during bonus                                    
                                    break;
                            }
                            //FireBall symbol update
                            if ($gameType == 'normal')
                            {
                                for($m = 1; $m < 6; $m++)
                                {
                                    ...
                                    for($n = 7; $n >= 0; $n--)
                                    {
                                        if (\intval($reels['reel'.$m][$n])  >= 18)
                                        {
                                            ...
                                            if ($currentTotalWin < $totalGuess)
                                            {
                                                ...
                                                if ($gameType == 'bonus')
                                                {
                                                    // if ($slotSettings->jpgs[0]->balance >= $slotSettings->jpgs[0]->pay_sum && !$isMega && !$isMajor && !$isMinor && !$isMini)
                                                    // {
                                                    //     $reels['reel'.$m][$n] = '6';
                                                    //     $isMega = true;
                                                    ...
                                                    //     $reels['reel'.$m][$n] = '4';
                                                    //     $isMinor = true;
                                                    // }else if ($slotSettings->jpgs[3]->balance >= $slotSettings->jpgs[3]->pay_sum && !$isMega && !$isMajor && !$isMinor && !$isMini)
                                                    // {
                                                    //     $reels['reel'.$m][$n] = '3';
                                                    //     $isMini = true;
                                                    // }
                                                ...
                                                {
                                                    if (\intval($reels['reel'.$m][$n]) <= 116)
                                                    {
                                                        $currentTotalWin += \intval($totalBet/50) * (50 +  (\intval($reels['reel'.$m][$n])-18)*25);
                                                    }else 
                                                    {
                                                        $currentTotalWin += \intval($totalBet/50) * 5000;
                                                    }
                            ...
                            if ($gameType == "normal")
                            {
                                ...
                                if ($fsCnt >= 3) //transit to freespin mode
                                {
                                    $bounsId = 1;
                                    for ($m = 0; $m < 5; $m++)
                                    {
                                        for($n = 0; $n < 4; $n++)
                                        {
                                            $reelSymbols[$m][$n] = $reelIndex[\rand(0,\count($reelIndex) - 1)]; //add symbol above 4 lines
                                        }
                                        for($n = 0; $n < 4; $n++)
                                        {
                                ...
                                if ($fbCnt >= 4) //transit to bonus mode
                                {
                                    for ($m = 0; $m < 5; $m++)
                                    {
                                        for($n = 0; $n < 4; $n++)
                            ...
                            }else if ($gameType == "freespin") //check freespin count
                            {
                                $fsCnt = 0;
                                for($m = 1; $m < 6; $m++)
                                {
                                ...
                                if ($fsCnt >= 3) //add freespin cnt
                                {
                                    $freeSpinTotalTimes += 5;
                                    $freeSpinAddTimes = 5;
                                }
                            }
                            
                            if ($gameType == "normal" || $gameType == "freespin")   //calculate total money
                            {
                                $step = 1;
                                if ($gameType == "freespin")
                                {
                                ...
                                if ($gameType == "freespin") //calculate fire ball symbol
                                {
                                    $fireBallSum = 0;
                                    for($a = 1;$a <= 5;$a++)
                                    {
                                        for($b = 0; $b < 8; $b++)
                                        {
                                            if (intval($reels['reel'.$a][$b]) >= 18 )
                                            {
                                                ...
                                                {
                                                    $fireBallValue += \intval($totalBet/50) * (50 +  (\intval($reels['reel'.$a][$b])-18)*25);
                                                }else 
                                                {
                                                    $fireBallValue += \intval($totalBet/50) * 5000;
                                                }
                                ...
                                if ($gameType == "freespin" || $i > 1900 || ($i >= 1800 && $totalWin == 0) || ($totalWin <=  $winTypeTmp[1] && $totalWin > 0.7 * $winTypeTmp[1]) || ( $winTypeTmp[1] == 0 && $totalWin == 0))
                                {
                                    ...
                                    for ($m = 0; $m < 5; $m++)
                                    {
                                        $n = 0;
                                        $index = 0;
                                        if(($bounsId == 1 || $bounsId == 2) && $freeSpinTotalTimes - $freeSpinTimes > 0)
                                        {
                                            $n = -4;
                                        }
                                        for(; $n < 4; $n++)
                                    ...
                                    for($b = 7; $b >= $topLimit; $b--)
                                    {
                                        ...
                                        else if (\intval($fireLinkTable[$a][$b]) == 6)
                                        {
                                            ...
                                            \VanguardLTE\StatGame::create([
                                                ...
                                                'percent_jpg' => 0, 
                                                'profit' => 0, 
                                                'shop_id' => intval($user->shop_id),
                                                'reels' => 'prize1'
                                            ]);

                                            $jackpotResponse = '{"pid": 10335,"jp_id": 3,"jpScore":'.($slotSettings->jpgs[0]->balance).'}'; ///////////
                                        ...
                                        }else if (\intval($fireLinkTable[$a][$b]) == 5)
                                        {
                                            ...
                                            $jackpotResponse = '{"pid": 10335,"jp_id": 2,"jpScore":'.($slotSettings->jpgs[1]->balance).'}'; ///////////
                                        ...
                                        }else if (\intval($fireLinkTable[$a][$b]) == 4)
                                        {
                                            ...
                                            $jackpotResponse = '{"pid": 10335,"jp_id": 1,"jpScore":'.($slotSettings->jpgs[2]->balance).'}'; ///////////
                                        ...
                                        }else if (\intval($fireLinkTable[$a][$b]) == 3)
                                        {
                                            ...
                                            $jackpotResponse = '{"pid": 10335,"jp_id": 0,"jpScore":'.($slotSettings->jpgs[3]->balance).'}'; ///////////
                        ...
                        if ($bounsId == 1 && $scatterLst != [-1,-1,-1,-1,-1]) //scatter 추가
                        {
                        ...
                        //
                        break;
                    case "wait_bonus": //freespin start reset
                        $gameType = "freespin";
                        $slotSettings->SetGameData('gameType',$gameType);
                        $betIndex = $slotSettings->GetGameData('betIndex');
                        switch($betIndex)
