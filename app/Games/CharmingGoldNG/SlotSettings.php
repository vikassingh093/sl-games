...
namespace VanguardLTE\Games\CharmingGoldNG
{
    ...
    class SlotSettings extends BasicSlotSettings
    {
        ...
        {
            $rpResult = [];
            for( $i = 0; $i < count($rp) - 5; $i++ ) 
            {
                if( $rp[$i] == $symbol) 
                {
                    if( isset($rp[$i + 1 + 5]) && isset($rp[$i - 1]) ) 
                    {
        ...
        public function GetReelStrips($winType, $slotEvent)
        {
                ...
                if($slotEvent == 'bet' && $winType == 'none')
                {
                    ...
                    if(rand(0, 100) < 30)
                    {
                        if(rand(0, 10) < 5)
                        {
                            $cnt = rand(3, 5);
                            for($i = 0; $i < $cnt; $i++)
                                $reels['reel'.rand(1,5)][rand(0,2)] = $sphere;
                        }
                        else
                        {
                            $reels['reel'.rand(1,5)][rand(0,2)] = $scatter;
                            $reels['reel'.rand(1,5)][rand(0,2)] = $scatter;
                        }
                    }
            ...
            else 
            {
                ...
                if(rand(0, 100) < 30)
                {
                    $reels['reel'.rand(1,5)][rand(0,$row_cnt)] = $sphere;
                    $reels['reel'.rand(1,5)][rand(0,$row_cnt)] = $sphere;
                }                
                
                if(rand(0, 100) < 10)
                    $reels['reel3'][rand(0, 2)] = $burst;
            }
        ...
        //             for($i = 0; $i < 5; $i++)
        //             {
        //                 $reelStrip = 'reelStrip' . ($i+1);
        //                 if($i < $cnt)
        //                 {
        //                     $prs[$i + 1] = $this->GetRandomScatterPos($this->$reelStrip, $sym);
        //                 }
        //                 else
        //                 {
        //                     $prs[$i + 1] = mt_rand(0, count($this->$reelStrip) - 5);
        //                 }
        //             }
        //         }
        //         else
