                        ...
                        for($i = 0; $i <= 2000 ; $i++)
                        {
                            ...
                            switch($gameType)
                            {
                                case "normal":
                                    ...
                                    {
                                        $reels = $slotSettings->GetReelStrips('freespin',0); //generate reel with freespin
                                        $slotSettings->SetGameData('totalGuess',$winTypeTmp[1]);
                                    }else if ($winTypeTmp[0] == "bonus")
                                    {
                                        $reels = $slotSettings->GetReelStrips('bonus',0); //genereate reel with bonus symbol
                                        $slotSettings->SetGameData('totalGuess',$winTypeTmp[1]);
                                    }
                                    break;
                                case "freespin":
                                    $reels = $slotSettings->GetReelStrips('none',1); //generate reel during freespin
                                    break;
                                case "bonus":
                                    $reels = $slotSettings->GetReelStrips('none',2); //generate reel during fireball                                    
                                    break;
                            }
                            //update fireball symbol
                            if ($gameType == 'normal')
                            {
                                for($m = 1; $m < 6; $m++)
                                {
                                    for($n = 0; $n < 4; $n++)
                                    {
                                        if (\intval($reels['reel'.$m][$n])  >= 18)
                                        {
                                            ...
                                            if ($fireBallPercent >= 0 && $fireBallPercent < 96)
                                            {
                                                $reels['reel'.$m][$n] = strval(\rand(18,23));
                                            }else if ($fireBallPercent >= 90 && $fireBallPercent < 97)
                                            {
                            ...
                            if ($gameType == "normal")
                            {
                                ...
                                if ($fsCnt >= 3) //to the freespin
                                {
                                    ...
                                    for ($m = 0; $m < 5; $m++)
                                    {
                                        for($n = 0; $n < 4; $n++)
                                        {
                                            $reelSymbols[$m][$n] = $reelIndex[\rand(0,\count($reelIndex) - 1)]; //add symbol to top 4 lines
                                        }
                                        for($n = 0; $n < 4; $n++)
                                        {
                            ...
                            }else if ($gameType == "freespin") //check freespin symbol count
                            {
                                $fsCnt = 0;
                                for($m = 1; $m < 6; $m++)
                                {
                            ...
                            if ($gameType == "normal" || $gameType == "freespin")  
                            {
                                ...
                                if ($gameType == "freespin") //calc fireball symbol
                                {
                                    $fireBallSum = 0;
                                    for($a = 1;$a <= 5;$a++)
                                    {
                                ...
                                if ($gameType == "freespin" || $i > 1900 || ($i >= 1800 && $totalWin == 0) || ($totalWin <=  $winTypeTmp[1] && $totalWin > $winTypeTmp[1] * 0.7) || ( $winTypeTmp[1] == 0 && $totalWin == 0))
                                {
