...
namespace VanguardLTE\Games\BoilingPotsYGG
{
    ...
    class SlotSettings extends BasicSlotSettings
    {
        ...
        public function __construct($sid, $playerId)
        {
            ...
            $this->Paytable['SCATTER'] = [0, 0, 0, 0, 0, 0];
            $this->Paytable['WILD'] = [0, 0, 0, 0, 0, 0];
            $this->Paytable['ARROW'] = [0, 0, 0, 0, 0, 0];
            $this->Paytable['HIGH_1'] = [0, 0, 0, 40, 80, 200];
            $this->Paytable['HIGH_2'] = [0, 0, 0, 20, 40, 80];
            $this->Paytable['HIGH_3'] = [0, 0, 0, 14, 30, 60];
            $this->Paytable['HIGH_4'] = [0, 0, 0, 10, 14, 30];
            $this->Paytable['HIGH_5'] = [0, 0, 0, 10, 14, 30];
            $this->Paytable['LOW_1'] = [0, 0, 0, 4, 6, 10];
            $this->Paytable['LOW_2'] = [0, 0, 0, 4, 6, 10];
            $this->Paytable['LOW_3'] = [0, 0, 0, 4, 6, 10];
            $this->Paytable['LOW_4'] = [0, 0, 0, 4, 60, 10];
            $this->Paytable['LOW_5'] = [0, 0, 0, 4, 6, 10];
        ...
        public function GetSpinSettings($garantType = 'bet', $bet, $lines, $isBuyFreespin)
        {
            $this->AllBet = $bet * $lines;
            return $this->SpinSettings($garantType, $this->AllBet, $isBuyFreespin);
        ...
        public function GetRandomScatterPos($rp, $endCnt, $sb)
        {
            ...
            for( $i = 0; $i < count($rp); $i++ ) 
            {
                if( $rp[$i] == $sb ) 
                {
                    ...
                    if( isset($rp[$i - 1 + $endCnt]) && isset($rp[$i - 6]) ) 
                    {
                        array_push($rpResult, $i - 1);
                    }
                    if(isset($rp[$i - 2 + $endCnt]) ) 
                    {
        ...
        {
            $filename = base_path() . '/app/Games/BoilingPotsYGG/'.$reelName.'.txt';
            ...
            if($winType != 'bonus')
            {
                if($winType == 'win')
                {
                    $syms = ["HIGH_1","HIGH_2","HIGH_3","HIGH_4","HIGH_5","LOW_1","LOW_2","LOW_3","LOW_4","LOW_5"];
                    $symb = $syms[rand(0, 9)];
                    for($i = 0; $i < 5; $i++) 
                    {
                        $prs[$i + 1] = $this->GetSymbolPos($reelStrips->reels[$i], $symb, $endLen);
                    }
                }
                else
                {
        ...
        public function GetNoWinSpin($reelName, $boardHeight)
        {
            $isWin = true;            
            ...
            {               
                ...
                for( $j = 0; $j <count($this->SymbolGame); $j++ ) 
                {
                    ...
                    $mul1 = $this->getMultiplier($reels['reel1'], $csym, $boardHeight);
                    $mul2 = $this->getMultiplier($reels['reel2'], $csym, $boardHeight);
                    $mul3 = $this->getMultiplier($reels['reel3'], $csym, $boardHeight);
                    $mul4 = $this->getMultiplier($reels['reel4'], $csym, $boardHeight);
                    $mul5 = $this->getMultiplier($reels['reel5'], $csym, $boardHeight);
                    $tmpWin = 0;
