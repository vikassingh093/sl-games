    ...
    class Server
    {
        public function get($request, $game)
        {
            ...
            try
            {
                ...
                //18:50 ~ 116:2500 117:5000
                $fireBallScore = array(18 => 50, 19 => 75, 20 => 100, 21 => 125, 22 => 150, 23 => 200, 24 => 250, 25 => 375, 26 => 500, 27 => 625, 28 => 750, 29 => 1000, 30 => 2500, 31 => 5000);
                //$freeRatioArry = [[10,15,25],[8,10,15],[5,8,10],[3,5,8],[2,3,5]];
                $freeRatioArry = [[10,15,10],[8,10,8],[5,8,10],[3,5,8],[2,3,5]];
                ...
                switch($type)
                {
                    ...
                    case "choice_card":
                        switch($body)
                        {
                            case '0':
                                $freeCnt = 5;
                                $freeRatio = $freeRatioArry[0][\rand(0,\count($freeRatioArry[0]) -1)];
                                break;
                            case '1':
                                $freeCnt = 7;
                                $freeRatio = $freeRatioArry[1][\rand(0,\count($freeRatioArry[1]) -1)];
                                break;
                            case '2':
                                $freeCnt = 10;
                                $freeRatio = $freeRatioArry[2][\rand(0,\count($freeRatioArry[2]) -1)];
                                break;
                            case '3':
                                $freeCnt = 15;
                                $freeRatio = $freeRatioArry[3][\rand(0,\count($freeRatioArry[3]) -1)];
                                break;
                            case '4':
                                $freeCnt = 20;
                                $freeRatio = $freeRatioArry[4][\rand(0,\count($freeRatioArry[4]) -1)];
                                break;
                            default:
                                $freeCnt = 5;
                                $freeRatio = $freeRatioArry[0][\rand(0,\count($freeRatioArry[0]) -1)];
                        ...
                        $slotSettings->SetGameData('freeSpinTotalTimes',$freeCnt);
                        $slotSettings->SetGameData('freeRatio',$freeRatio);
                        $bodyResponse = '{"cbCardIndex":'.$freeCnt.'}';
                        $response = '{"main":200,"type": 214,"body":'.$bodyResponse.'}'; //SUB_S_PLAYER_DATA:214
                        array_push($returnResponse,$response);
                        break;
                    case "spin":
                        ...
                        $freeRatio = $slotSettings->GetGameData('freeRatio');
