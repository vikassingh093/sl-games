                        ...
                        switch( $reqId ) 
                        {
                            ...
                            case 'AuthRequest':
                                ...
                                if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                                {
                                    ...
                                    $restoreString = ',"restoredGameCode":"273","lastResponse":{"spinResult":{"type":"SpinResult","rows":[' . $rp2 . ']},"freeSpinsTotal":"' . $fTotal . '","freeSpinRemain":"' . $fRemain . '","totalBonusWin":"' . $fBonusWin . '","state":"FreeSpins","expandingSymbols":["1"]}';
                                }
                            ...
                            case 'SpinRequest':
                                ...
                                if($postData['slotEvent'] == 'freespin')
                                {
                                    ...
                                    if($freeSpinRemain < 3 && $bonusWin0 == 0)
                                        $winType = 'win';
                                    $spinWinLimit = $spinWinLimit > ($betLine * $lines) * 5 ? ($betLine * $lines) * 5 : $spinWinLimit;
                                    if(rand(0, 100) < 30)
                                    {
                                        $winType = 'none';
                                        $spinWinLimit = 0;
                                ...
                                for( $i = 0; $i <= 300; $i++ ) 
                                {
                                    ...
                                    // {
                                    //     //reel debug test
                                    //     $reels['reel1'] = [1,2,1,-1,1];
                                    //     $reels['reel2'] = [4,5,2,5,4];
                                    //     $reels['reel3'] = [4,5,3,2,5];
                                    //     $reels['reel4'] = [1,2,3,1,2];
                                    //     $reels['reel5'] = [6,7,3,-1,6];
                                    // }
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            $wscc = 0;
                                            $cl = -1;
                                            if( ((in_array($csym, $reels['reel1']) || in_array($wild[0], $reels['reel1'])) &&
                                                (in_array($csym, $reels['reel2']) || in_array($wild[0], $reels['reel2'])) &&
                                                (in_array($csym, $reels['reel3']) || in_array($wild[0], $reels['reel3']))) || 
                                                ((in_array($csym, $reels['reel3']) || in_array($wild[0], $reels['reel3'])) &&
                                                (in_array($csym, $reels['reel4']) || in_array($wild[0], $reels['reel4'])) &&
                                                (in_array($csym, $reels['reel5']) || in_array($wild[0], $reels['reel5']))))
                                                {
                                                    $cl = 0;
                                                    $wscc = 3;
                                                }
                                            ...
                                            for( $k = 0; $k < count($linesId[$cl]); $k++ ) 
                                            {
                                                ...
                                                if( $wscc == 3 ) 
                                                {
                                                    ...
                                                    if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                                    {
                                                        ...
                                                        {
                                                            $cWins[$k] = $tmpWin;
                                                            $tmpStringWin = '{"type":"LineWinAmount","selectedLine":"' . $k . '","amount":"' . $tmpWin . '","wonSymbols":'.json_encode($wonSymbols).'}';
                                                        }
                                                    }
                                                }
                                                if( $wscc == 4 ) 
                                                {
                                                    ...
                                                    {
                                                        $s[0] = $reels['reel2'][$linesId[$cl][$k][1] - 1];
                                                        $s[1] = $reels['reel3'][$linesId[$cl][$k][2] - 1];
                                                        $s[2] = $reels['reel4'][$linesId[$cl][$k][3] - 1];
                                                        $s[3] = $reels['reel5'][$linesId[$cl][$k][4] - 1];
                                                        $s[4] = -1;
                                                        $p0 = 0;
                                                        $p1 = $linesId[$cl][$k][1] - 1;
                                                        $p2 = $linesId[$cl][$k][2] - 1;
                                    ...
                                    {
                                        $freeSpinMul = $rowScatters[2] * $rowScatters[3] * $rowScatters[4];
                                        $newFreespinCount = $slotSettings->slotFreeCount * $freeSpinMul > 128 ? 128 : $slotSettings->slotFreeCount * $freeSpinMul;
                                        $gameState = 'FreeSpins';
                                        $scw = '{"type":"Bonus","bonusName":"FreeSpins","params":{"freeSpins":"' . $newFreespinCount . '"},"amount":"' . $slotSettings->FormatFloat($scattersWin) . '","wonSymbols":[' . implode(',', $scattersPos) . ']}';
                                    ...
                                    if($totalWin <= $spinWinLimit && (($totalWin > 0 && $winType != 'none') || ($winType == 'bonus' && $scattersCount > 2)))
                                    {
                                        $spinAcquired = true;
                                ...
                                if( $scattersCount >= 3 ) 
                                {
                                    ...
                                    {
                                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeGames', $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') + $newFreespinCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeStartWin', $totalWin);
                                        $slotSettings->SetGameData($slotSettings->slotId . 'BonusWin', $totalWin);
                                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeGames', $newFreespinCount);
                                    }
                                }
