...
namespace VanguardLTE\Http\Controllers\Web\Backend
{
    ...
    class ShopsController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function edit($shop)
        {
            ...
            $bounce = \VanguardLTE\BounceBack::where('shop_id', '=', $user->shop_id)->get();  
            $zones = \VanguardLTE\TimeZone::all();
            $selectedZone = $user->timezone;

            $jackpots = \VanguardLTE\JPG::where('shop_id', '=', $user->shop_id)->get();
            return view('backend.shops.edit', compact('shop', 'directories', 'categories', 'cats', 'blocks', 'activity', 'selector', 'contact_info', 'bounce', 'user', 'zones', 'selectedZone', 'jackpots'));
        }
        ...
        public function update(\Illuminate\Http\Request $request, \VanguardLTE\Repositories\Session\SessionRepository $sessionRepository, \VanguardLTE\Shop $shop)
        {
            ...
            $shop->update(['percent' => $req_data['rtp']]);
            ...
            //save bounces
            // $bounceParams = $req_data['bounceback_params'];
            // for($i = 1; $i <= 4; $i++)
            // {
            //     $value = $bounceParams['value'][$i];
            //     $type = $bounceParams['type'][$i];
            //     $field_name = $type == 2 ? 'fixed_limit' : 'percent_limit';
            //     \VanguardLTE\BounceBack::where('shop_id', '=', $shop->id)
            //                             ->where('name', '=', 'bounce'.$i)
            //                             ->update([$field_name => $value, 'mode' => $type]);
            // }
