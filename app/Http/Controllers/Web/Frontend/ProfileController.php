...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    ...
    class ProfileController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function updatePhone(\Illuminate\Http\Request $request)
        {
            $data = $request->all();
            $phone = $data['phone'];
            $this->users->update($this->theUser->id, ['phone' => $phone]);
            return response()->json(['success' => trans('app.profile_updated_successfully')], 200);
        }

        public function updateNewPassword(\Illuminate\Http\Request $request)
        {
            $data = $request->all();
            ...
            if($password != $password_confirm)
            {
                return response()->json(['error' => 'password conform does not match'], 200);    
            }
