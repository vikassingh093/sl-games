...
export class JackpotUnit extends PIXI.Sprite{
	...
	constructor(type:string) {
		...
		this.back = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from("/images/screens/elements/jackpot/bg_jackpot.png")));
		this.back_light = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from("/images/screens/elements/jackpot/bg_jackpot_light.png")));
		this.back_light.alpha = 0;
		//
		this.color_btn = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from(`/images/screens/elements/jackpot/bg_${type.toLowerCase()}.png`)));
		this.color_btn_light = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from(`/images/screens/elements/jackpot/bg_${type.toLowerCase()}_light.png`)));
		this.color_btn_light.visible = false;
		this.color_btn_light.x = this.color_btn.x = 20;
		this.color_btn_light.y = this.color_btn.y = 11;


		const txt = this.cont.addChild(new PIXI.Text(type.toUpperCase(), styletext));
		txt.x = 80 - (txt.width/2);
		txt.y = 30;
		//
