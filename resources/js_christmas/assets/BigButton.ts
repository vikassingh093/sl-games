...
export class BigButton extends PIXI.Sprite{
	...
	constructor(idd:number) {
		...
		this.pic.addChild(new PIXI.Sprite(PIXI.Texture.from("/images_christmas/buttons/back_blue.png")));

		//this.yellow_back = this.pic.addChild(new PIXI.Sprite(PIXI.Texture.from("/images_christmas/buttons/back_yellow.png")));
		//this.yellow_back.visible = false;

		const json0 = PIXI.Loader.shared.resources['/images_christmas/buttons/anim1.json'].spritesheet;
		const array0:any = [];
		if(json0) {
		...
		this.yellow_ball = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from("/images_christmas/buttons/round_yellow.png")));
		this.yellow_ball.visible = false;
		this.yellow_ball.x = 230;
		this.yellow_ball.y = 8;

		this.blue_ball = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from("/images_christmas/buttons/round_blue.png")));
		this.blue_ball.visible = false;
		this.blue_ball.x = 230;
		this.blue_ball.y = 8;

		this.text_blue = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from(`/images_christmas/buttons/b${idd}text_b.png`)));
		this.text_blue.visible = false;
		this.text_blue.x = 33;
		this.text_blue.y = 35;

		this.text_yellow = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from(`/images_christmas/buttons/b${idd}text_y.png`)));
		this.text_yellow.visible = false;
		this.text_yellow.x = 33;
		this.text_yellow.y = 35;

		const logo = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from(`/images_christmas/buttons/b${idd}logo.png`)));
		logo.x = 230;
		logo.y = 8;
