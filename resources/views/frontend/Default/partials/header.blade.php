...
<div class="container" style="margin-bottom:45px;height: 200px;">
    ...
    <div class="header @yield('add-header-class')">
        ...
        <div class="mobile-balance">
            ...
            <span class="info-value">{{ Auth::check() ? number_format(auth()->user()->balance, 2,".","") : "--" }} {{ $currency }}</span></li>
        </div>
        <div class="mobMenu">
            @if(Auth::check())
                <img src="/frontend/Default/img/badges64x64/badge-{{ auth()->user()->badge() }}.png" alt="" width="32" height="32">
            @else
                <img src="/frontend/Default/img/badges64x64/badge-00.png" alt="" width="32" height="32">
            @endif
        </div>
        <div class="mobile-menu">
            <div class="mobile-menu__wrap">
                <div class="mobile-menu__item">
                    <div class="mobile-menu__acc">
                        <div class="mobile-menu__item-acc-img">
                            ...
                            @else
                                <img src="/frontend/Default/img/badges64x64/badge-{{ Auth::check() ? auth()->user()->badge() : '00' }}.png" alt="" width="72" height="72">
                            @endif
                            <div class="mobile-menu__item-acc-rating">{{ Auth::check() ? auth()->user()->username : "" }}</div>
                        </div>
                        <ul class="mobile-menu__item-acc-info">
                            ...
                            <li class="tooltip-btn bonusMenu">
                                ...
                                @endif
                                <span class="info-value">{{ Auth::check() ? number_format( (auth() ->user()->tournaments + auth() ->user()->happyhours + auth()->user()->refunds + auth() ->user()->progress + auth() ->user()->daily_entries + auth() ->user()->invite + auth() ->user()->welcomebonus + auth() ->user()->smsbonus + auth() ->user()->wheelfortune), 2,".","") : "--" }} {{ $currency }}</span>
                            </li>
                            ...
                            <li class="tooltip-btn wagerMenu">
                                ...
                                <span class="info-value">{{ Auth::check() ? number_format( (auth() ->user()->count_tournaments + auth() ->user()->count_happyhours + auth()->user()->count_refunds + auth() ->user()->count_progress + auth() ->user()->count_daily_entries + auth() ->user()->count_invite + auth() ->user()->count_welcomebonus + auth() ->user()->count_smsbonus + auth() ->user()->count_wheelfortune), 2,".","") : "--" }} {{ $currency }}</span>
                            </li>

                            <li class="tooltip-btn refunds-icon">
                                @if ( Auth::check() && $refund && auth()->user()->present()->refunds > 0 && auth()->user()->present()->balance <= $refund->min_balance )
                                ...
                                @else
                                    ...
                                    <span class="info-value refunds">{{ Auth::check() ? number_format(auth()->user()->refunds, 2,".","") : "--" }} {{ $currency }}</span>
                                @endif
                            </li>
                ...
                <div class="mobile-menu__item">
                    ...
                    @else
                    <a href="javascript:;" data-name="modal-login" class="btn btn--logout modal-btn">Login</a>
                    <a href="javascript:;" data-name="modal-register" class="btn btn--logout modal-btn">Register</a>
                    @endif
                </div>
                <div class="mobile-menu__item mobile-menu-list">
                ...
                <ul class="mobile-menu__list">
                    ...
                    @endif
                    @if( Auth::check() && auth()->user()->shop->getBonusesList() )
                    <li class="mobile-menu__list-item">
                        <a href="{{ route('frontend.bonuses') }}" class="mobile-menu__list-link">Bonuses</a>
                    </li>
                    @endif
                    @if( Auth::check() && count($rules) && auth()->user()->shop->hasActiveRules())
    ...
    <div class="auth-btn-group">
        <a href="javascript:;" data-name="modal-login" class="auth-btn checkAgreed btn modal-btn">Login</a>
        <a href="javascript:;" data-name="modal-register" class="auth-btn checkAgreed btn modal-btn">Register</a>
    </div>
    @endif
</div>
